(ns app.core
  (:require
   ...
   ["codejar" :as Codejar]
   [app.components.molstar :refer [mvs-sample molstar-viewer-mvsdata]]
   [app.components.editor :refer [ClojureEditor]]))
  ...
  (let [[value set-value!] (uix/use-state "")]
    ($ :input.text-input
       {:value value
        ...
        :on-change (fn [^js e] (set-value! (.. e -target -value)))
        :on-key-down (fn [^js e]
                       (when (= "Enter" (.-key e))
                         (set-value! "")
...
(defui app []
  (let [[mvs-data, set-mvs-data] (uix/use-state mvs-sample)
        todos (uix.rf/use-subscribe [:app/todos])]
    ($ :.app
       ($ header)
       ($ ClojureEditor)
       ($ molstar-viewer-mvsdata {:mvs-data mvs-data})
